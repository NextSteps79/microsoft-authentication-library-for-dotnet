...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public async Task CertificateOverrideAsync()
        {
            using (var httpManager = new MockHttpManager())
            {
                ...
                Func<string, CancellationToken, Task<IReadOnlyList<KeyValuePair<string, string>>>> clientAssertionOverride =
                    (tokenEndpoint, ct) =>
                    {
                        Assert.AreEqual("https://login.microsoftonline.com/tid/oauth2/v2.0/token", tokenEndpoint);
                        IReadOnlyList<KeyValuePair<string, string>> list = new List<KeyValuePair<string, string>>()
                        {
